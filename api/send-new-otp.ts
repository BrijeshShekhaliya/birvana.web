...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const oldEmail = user.email || 'your current email';
    ...
    if (newEmail.toLowerCase().trim() === oldEmail.toLowerCase().trim()) {
      return res.status(400).json({ error: 'You cannot use the same email as your current one' });
    }

    // Optional: check if email already exists in profiles
    ...
    if (dbError) {
      console.error('Database error storing new OTP:', dbError);
      return res.status(500).json({ error: 'Failed to generate verification code' });
    }

    // Send confirmation OTP to new email
    const subject = `${otp} is your Birvana confirmation code`;
...
<body style="margin: 0; padding: 0; background-color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #171717; -webkit-font-smoothing: antialiased;">
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color: #ffffff; width: 100%; margin: 0 auto; padding: 40px 20px;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width: 480px; margin: 0 auto; text-align: left;">
          ...
          <tr>
            <td style="padding-bottom: 24px;">
              <h2 style="margin: 0; font-size: 24px; font-weight: 600; color: #171717; letter-spacing: -0.5px;">Confirm your new email address</h2>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding-bottom: 32px;">
              <p style="margin: 0; font-size: 16px; line-height: 24px; color: #52525B;">
                You are receiving this email because a request was made to set this address as the new primary contact email for a Birvana user account.
              </p>
              <p style="margin: 16px 0 0 0; font-size: 16px; line-height: 24px; color: #52525B;">
                Enter the verification code below in your mobile app to confirm this change. <strong>Do not share this code with anyone.</strong>
              </p>
            </td>
          </tr>
    ...
    await logEmail(newEmail, 'new_email_verification_otp', subject);

    return res.status(200).json({ success: true, message: 'OTP sent successfully to new email' });
  } catch (error: any) {
    console.error('API Error in send-new-otp:', error);
